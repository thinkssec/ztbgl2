...
namespace Enterprise.Component.Control
{
    ...
    public class HeadMenuWebControls : WebControl
    {
        ...
        private string HeadMenuTemplateTxt = Utility.HeadMenuTemplateTxt;
        ...
        private string CreateButtonHtml()
        {
            ...
            if (_ButtonList != null && _ButtonList.Count > 0)
            {
                ...
                {
                    if (_ButtonList[i].ButtonVisible &&Enterprise.Service.Basis.Sys.SysUserPermissionService.CheckButtonPermission(_ButtonList[i].ButtonPopedom))
                    {
                        DispButtonNum++;
                        OnUrlJs = "";
                        ButtonIcon = "";
                        ...
                        sb.AppendFormat("{0}{1}</td><td width=\"5px\">&nbsp;</td>", ButtonTxt, Enterprise.Service.Basis.Sys.SysDictionaryService.TransTo(_ButtonList[i].ButtonName));
                    }
                }
                if (DispButtonNum == 0)
                    sb.Append("<td>&nbsp;</td>");
