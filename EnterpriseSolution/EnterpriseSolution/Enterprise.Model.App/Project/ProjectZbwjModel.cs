using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Enterprise.Model.App.Project
{
    /// <summary>
    /// 
	/// 创建人:代码生成器
	/// 创建时间:	2015/9/6 0:28:24
    /// </summary>
    [Serializable]
    public class ProjectZbwjModel : AppSuperModel
    {
        #region 代码生成器
        
			/// <summary>
			///
			/// </summary>
			public virtual string PROJID
			{
				get;
				set;
			}

			/// <summary>
			///附件文件名称
			/// </summary>
			public virtual string FNAMES
			{
				get;
				set;
			}

			/// <summary>
			///附件保存名称
			/// </summary>
			public virtual string FVIEWNAMES
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual int? SUBMITTER
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual DateTime? SUBMITTIME
			{
				get;
				set;
			}

			/// <summary>
			///发售招标文件起止时间
			/// </summary>
			public virtual string P1
			{
				get;
				set;
			}

			/// <summary>
			///招标文件澄清截止时间
			/// </summary>
			public virtual string P2
			{
				get;
				set;
			}

			/// <summary>
			///招标文件答疑
			/// </summary>
			public virtual string P3
			{
				get;
				set;
			}

			/// <summary>
			///投标有效期
			/// </summary>
			public virtual string P4
			{
				get;
				set;
			}

			/// <summary>
			///投标保证金
			/// </summary>
			public virtual string P5
			{
				get;
				set;
			}

			/// <summary>
			///投标文件份数 正本
			/// </summary>
			public virtual string P6
			{
				get;
				set;
			}

			/// <summary>
			///副本
			/// </summary>
			public virtual string P7
			{
				get;
				set;
			}

			/// <summary>
			///招标范围
			/// </summary>
			public virtual string P8
			{
				get;
				set;
			}

			/// <summary>
			///招标编号1
			/// </summary>
			public virtual string P9
			{
				get;
				set;
			}

			/// <summary>
			///招标编号2
			/// </summary>
			public virtual string P10
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string P11
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string P12
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string P13
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string Z1
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string Z2
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string Z3
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string Z4
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string Z5
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string Z6
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string Z7
			{
				get;
				set;
			}

			/// <summary>
			///
			/// </summary>
			public virtual string Z8
			{
				get;
				set;
			}

        #endregion
    }

}
