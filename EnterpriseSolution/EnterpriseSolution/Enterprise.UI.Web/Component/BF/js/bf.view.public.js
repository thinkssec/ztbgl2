...
LineXML.prototype = {
    ...
    view: function () {
        ...
        //alert("g.strokeweight==" + g.strokeweight + "=====g.strokecolor====" + g.strokecolor);
        //add by qw 2013.5.20 start 控制线的样式
        if (runPaths.indexOf(g.xBaseMode + "-" + g.wBaseMode + ",") > -1) {
            //已运行路径
            h.setAttribute("strokecolor", "#ff00ff"); //加亮显示
        }
        else {
            h.setAttribute("strokecolor", "#999999"); //缺省为未运行路径
        }
        //end
        h.setAttribute("strokeweight", "2px");
    ...
    toHTML: function () {
        ...
        if (q.root) {
            ...
            for (var h = o; h--; ) {
                ...
                //add by qw 2013.5.20 start
                if (n == "mode") {
                    var nodes = runNodes.split(',');
                    var mTool = com.xjwgraph.Global.modeTool;
                    var mIndex = mTool.getModeIndex(l.modeDiv);
                    var imgSrc = $id("backImg" + mIndex).src;
                    var modeId = "module" + mIndex;
                    if (runNodes.indexOf(modeId + ",") == -1) {
                        $id("backImg" + mIndex).src = imgSrc.replace("Node.", "Node_grey.");
                    }
                    else if (nodes[nodes.length - 2] == modeId) {
                        $id("backImg" + mIndex).src = imgSrc.replace("Node.", "Node_red.");
                    }
                }
                //end
            }
...
//        var nodes = runNodes.split(',');
//        var aa = global.modeMap;
//        var lineMaps = global.lineMap;
//        var mTool = global.modeTool;
//        var modeKeys = aa.getKeys();
//        var modeKeysLength = modeKeys.length;
//        for (var j = modeKeysLength; j--; ) {
//            alert(modeKeys[j]);
////            var u = aa.get(modeKeys[j]);
////            var modeDiv = $id(u.id);
////            var mIndex = mTool.getModeIndex(modeDiv);
////            var imgSrc = $id("backImg" + mIndex).src;
////            if (runNodes.indexOf(modeKeys[j]+",") == -1) {
////                $id("backImg" + mIndex).src = imgSrc.replace("Node.","Node_grey.");
////            }
////            else if (nodes[nodes.length-2] == modeKeys[j]) {
////                $id("backImg" + mIndex).src = imgSrc.replace("Node.","Node_red.");
////            }
//        }