...
{
    public partial class DmwxJdList : Enterprise.Service.Basis.BasePage
    {
        ...
        private void BindGrid()
        {
            ...
            list = Service.GetList().Where(p=>p.PSTARTDATE>=st&&p.PENDDATE<=et&&p.SP2STATUS==1).OrderByDescending(p => p.PSTARTDATE).ToList();
        ...
        public void Bt_Search_OnClick(object sender, EventArgs e)
        {
            ...
            DmglPlanService Service = new DmglPlanService();
            var list = Service.GetList().Where(p=>p.SP2STATUS==1).OrderByDescending(p => p.PSTARTDATE).ToList();
            if (sType == "1")
                list = list.Where(p=>p.STATUS==3).ToList();
            if (sType == "0")
                list = list.Where(p => p.STATUS != 3 && p.SP2STATUS == 1).ToList();
        ...
        protected string GetStatus(object obj)
        {
            ...
            if (model.SP2STATUS == 1 && model.STATUS != 3) sName = "进行中";
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            if (m.SP2STATUS == 1&&m.STATUS!=3)
            {
                btnStrs += string.Format("<a  href='DmwxJdEdit.aspx?Cmd=Edit&PID={0}'><img alt=\"\" src=\"/Resources/Common/img/icon/application_edit.png\" border=\"0\" /></a>", id);
                //btnStrs += "&nbsp;&nbsp;|&nbsp;&nbsp;";
