...
{
    public partial class DmwxJsList : Enterprise.Service.Basis.BasePage
    {
        ...
        private void BindGrid()
        {
            ...
            list = Service.GetList().Where(p=>p.PSTARTDATE>=st&&p.PENDDATE<=et&&p.STATUS>=4).OrderByDescending(p => p.PSTARTDATE).ToList();
        ...
        public void Bt_Search_OnClick(object sender, EventArgs e)
        {
            ...
            DmglPlanService Service = new DmglPlanService();
            var list = Service.GetList().Where(p=>p.STATUS>=4).OrderByDescending(p => p.PSTARTDATE).ToList();
            if (sType == "1")
                list = list.Where(p=>p.STATUS==5).ToList();
            if (sType == "0")
        ...
        protected string GetStatus(object obj)
        {
            ...
            if (model.STATUS == 4) sName = "完工验收";
            if (model.STATUS == 5) sName = "已结算";
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            if (m.STATUS==4)
            {
                btnStrs += string.Format("<a  href='DmwxJsEdit.aspx?Cmd=Edit&PID={0}'><img alt=\"\" src=\"/Resources/Common/img/icon/application_edit.png\" border=\"0\" /></a>", id);
                //btnStrs += "&nbsp;&nbsp;|&nbsp;&nbsp;";
