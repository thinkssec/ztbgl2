...
{
    public partial class DmwxPlanSp : BasePage
    {
        ...
        private void BindGrid()
        {
            ...
            IList<DmglPlanModel> list1 = aService.GetList().Where(p => p.STATUS==1&&p.SPSTATUS==0).ToList();
            GridView3.DataSource = list1;
            GridView3.DataBind();
            int ct = aService.GetList().Where(p => p.SPSTATUS == 0).Count();
        ...
        protected void btn_cancle_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in GridView3.Rows)
            {
                ...
                if (cb.Checked)
                { 
                    TextBox yj = (TextBox)row.Cells[5].FindControl("memoT4");
                    ...
                    md.SPCONTENT = yj.Text;
                    md.SPSTATUS = 2;
                    md.SPTIME = DateTime.Now;
            ...
            int ct = aService.GetList().Where(p => p.SPSTATUS == 0).Count();
            if (ct == 0) BfMsgService.UpdateMsgOver(MsgID);
            Utility.ShowMsgThenReloadTree(this.Page, "系统提示", "操作成功", "DmwxPlanSp.aspx");
        ...
        protected void btn_ok_Click(object sender, EventArgs e)
        {
            ...
            SysDutyModel dm = sd.GetList().Where(p => p.DNAME == "地面维修主要领导").FirstOrDefault();
            SysUserDutyService ssss = new SysUserDutyService();
            SysUserDutyModel spr2 = ssss.GetList().Where(p => p.DUTYID == dm.DUTYID).FirstOrDefault();
            Boolean t = true;
            foreach (GridViewRow row in GridView3.Rows)
            {
                CheckBox cb = (CheckBox)row.Cells[0].FindControl("cb_select");
                if (cb.Checked)
                {
                    ...
                    if (t)
                    {
                        sendMessageToSPR2(CommonTool.GetGuidKey(), spr2.USERID.Value);
