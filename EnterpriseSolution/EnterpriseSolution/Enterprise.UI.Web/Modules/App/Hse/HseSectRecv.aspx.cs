...
{
    public partial class HseSectRecv : Enterprise.Service.Basis.BasePage
    {
        ...
        protected void BtnSave_OnClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    TextBox present = (TextBox)row.Cells[5].FindControl("Txt_PRESENT");
                    TextBox completedate = (TextBox)row.Cells[6].FindControl("Txt_COMPLETEDATE");
                    Image m = (Image)row.Cells[6].FindControl("Img_Key1");
                    //Label btn = (Label)row.Cells[6].FindControl("Lb_Btn");
                    System.Web.UI.HtmlControls.HtmlInputHidden hid = (System.Web.UI.HtmlControls.HtmlInputHidden)row.Cells[6].FindControl("Hid_Key1");

                    string str_Id = GridView1.DataKeys[row.RowIndex].Value.ToString();
                    var md = dservice.GetList().FirstOrDefault(p => p.CKDID == str_Id);
                    md.PRESENT = present.Text;
                    md.COMPLETEDATE = completedate.Text.ToDateTime();
                    md.FNAMES = hid.Value;
            ...
            Response.Redirect("HseSectRecv.aspx?Cmd=Recv&CKID=" + Hid_CKID.Value, false);
        }
        ...
        protected void BtnAudit_OnClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                entity.SUBMITDATE = DateTime.Now;
                entity.SHR1 = User_SHR1.UserSelectValue.ToInt();
                entity.SH1SUBMITIME = DateTime.Now;
                entity.SHR2 = User_SHR2.UserSelectValue.ToInt();
                entity.SH1STATUS = 0;
                //entity.CTARGET = Txt_TARGET.Text;
                //entity.CLEVEL = Ddl_LEVEL.SelectedValue;
