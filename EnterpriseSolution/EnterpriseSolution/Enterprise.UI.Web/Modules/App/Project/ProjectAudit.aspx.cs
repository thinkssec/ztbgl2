...
{
    public partial class ProjectAudit : Enterprise.Service.Basis.BasePage
    {
        ...
        private void OnStart()
        {
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ProjectAudit.aspx?Cmd=Edit&PROJID=" + Hid_CKID.Value, false);
                }
            }
            else if (Cmd == "Audit") {
                OnBindData();
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectAudit.aspx?Cmd=Delete2&FID={0}&PROJID={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
        ...
        private string sendMessageToRecv(ProjectInfoModel model)
        {
            ...
            bfCtrl.SendNotificationMessage(
                ...
                Txt_SPR.SelectedValue.ToInt(),//接收人
                this.userModel,//当前用户
                string.Format("{0}-招标申请，需要您进行审批!", model.PROJNAME),//消息标题
                string.Format("/Modules/App/Project/ProjectAudit.aspx?Cmd=Audit2&PROJID={0}", model.PROJID),//消息内容
        ...
        protected void BtnNew_OnClick(object sender, EventArgs e)
        {
            ...
            Response.Redirect("ProjectAudit.aspx?Cmd=" + Cmd + "&PROJID=" + Hid_CKID.Value + "&MsgID="+MsgID, false);
            //Utility.ShowMsg(this.Page, "OK", this.Trans(_note), "ProjectInfoList.aspx?Cmd=Edit&CKID=" + Hid_CKID.Value);
        }
        /// <summary>
        /// 保存操作
