...
{
    public partial class ProjectAudit1 : Enterprise.Service.Basis.BasePage
    {
        ...
        private void OnStart()
        {
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ProjectAudit1.aspx?Cmd=Edit&PROJID=" + Hid_CKID.Value, false);
                }
            }
            else if (Cmd == "Audit1") {
                OnBindData();
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectAudit1.aspx?Cmd=Delete2&FID={0}&PROJID={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
        ...
        private string sendMessageToRecv(ProjectInfoModel model)
        {
            ...
            foreach (var item in dutylist)
            {
                ...
                bfCtrl.SendNotificationMessage(
                    ...
                    string.Format("/Modules/App/Project/ProjectAudit2.aspx?Cmd=Audit2&PROJID={0}", model.PROJID),//消息内容
        ...
        protected void BtnNew_OnClick(object sender, EventArgs e)
        {
            ...
            Response.Redirect("ProjectAudit1.aspx?Cmd=" + Cmd + "&PROJID=" + Hid_CKID.Value + "&MsgID="+MsgID, false);
            //Utility.ShowMsg(this.Page, "OK", this.Trans(_note), "ProjectInfoList.aspx?Cmd=Edit&CKID=" + Hid_CKID.Value);
        }
        /// <summary>
        /// 保存操作
