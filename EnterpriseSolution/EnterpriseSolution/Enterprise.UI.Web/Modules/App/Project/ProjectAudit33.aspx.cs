...
{
    public partial class ProjectAudit33 : Enterprise.Service.Basis.BasePage
    {
        ...
        private void OnStart()
        {
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ProjectAudit33.aspx?Cmd=Edit&PROJID=" + Hid_CKID.Value, false);
                }
            }
            else if (Cmd == "Audit33") {
                OnBindData();
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectAudit33.aspx?Cmd=Delete2&FID={0}&PROJID={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
        ...
        private string sendMessageToRecv(ProjectTpinfoModel model)
        {
            ...
            IList<SysUserModel> list = new List<SysUserModel>();
            SysUserService suer = new SysUserService();
            list = suer.GetList().Where(w => w.DEPTID == deptid&& w.ULOGINPASS == "1").ToList();
            foreach (var item in list)
            {
                ...
                bfCtrl.SendNotificationMessage(
                    ...
                    item.USERID,//接收人
                    this.userModel,//当前用户
                    string.Format("{0}-立项申请，需要"+deptn+"进行审批!", model.PROJNAME),//消息标题
                    string.Format("/Modules/App/Project/ProjectAudit43.aspx?Cmd=Audit43&PROJID={0}", model.PROJID),//消息内容
        ...
        protected void BtnCancel_OnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                entity.STATUS = 2;
                entity.SPR2 = this.userModel.USERID;
                entity.SPYJ2 = Txt_SHYJ.Text;
                entity.RTN = this.userModel.USERID;
                entity.RTNSTATUS = "Audit33";
