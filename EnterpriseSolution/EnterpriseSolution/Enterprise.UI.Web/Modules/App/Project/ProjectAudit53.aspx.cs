...
{
    public partial class ProjectAudit53 : Enterprise.Service.Basis.BasePage
    {
        ...
        private void OnStart()
        {
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ProjectAudit53.aspx?Cmd=Edit&PROJID=" + Hid_CKID.Value, false);
                }
            }
            else if (Cmd == "Audit53") {
                OnBindData();
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectAudit53.aspx?Cmd=Delete2&FID={0}&PROJID={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
            ...
            if (model.SPR5 == null)
            {
                ...
                foreach (var item in dutylist)
                {
                    ...
                    bfCtrl.SendNotificationMessage(
                        ...
                        string.Format("/Modules/App/Project/ProjectAudit63.aspx?Cmd=Audit63&PROJID={0}", model.PROJID),//消息内容
