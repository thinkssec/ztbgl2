...
{
    public partial class ProjectAudit63 : Enterprise.Service.Basis.BasePage
    {
        ...
        private void OnStart()
        {
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ProjectAudit63.aspx?Cmd=Edit&PROJID=" + Hid_CKID.Value, false);
                }
            }
            else if (Cmd == "Audit63") {
                OnBindData();
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectAudit63.aspx?Cmd=Delete2&FID={0}&PROJID={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
        ...
        protected void BtnOk_OnClick2(object sender, EventArgs e)
        {
            try
            {
                ...
                ZbwjService.CreateLxsqTp(Hid_CKID.Value);
                if(1==entity.KINDID)
                    ZbwjService.CreateZbsqTp1(Hid_CKID.Value);
                if (2 == entity.KINDID)
                    ZbwjService.CreateZbsqTp2(Hid_CKID.Value);
                if (3 == entity.KINDID)
                    ZbwjService.CreateZbsqTp3(Hid_CKID.Value);
