...
namespace Enterprise.UI.Web.Modules.App.Project
{
    ...
    /// 项目列表页面
    /// </summary>
    public partial class ProjectInfoList3 : BasePage
    {
        #region 初始化参数区
        ...
        private void bindData()
        {
            ...
            ///end
            ProjectTpinfoService service = new ProjectTpinfoService();

            List<ProjectTpinfoModel> ls = service.GetList().Where(p=>p.STATUS>-2&&p.STATUS<=2).ToList();
        ...
        {
            ProjectTpinfoModel model = obj as ProjectTpinfoModel;
            ...
            switch (model.STATUS.ToRequestString())
            {
                ...
                case "0":
                    ...
                    //    new int[] { model.SHR.ToInt(),model.SPR.ToInt(),model.SPR2.ToInt(),model.SPR3.ToInt(),model.SPR4.ToInt(),model.SPR5.ToInt(),model.SPR6.ToInt()});;
                    sName = "待审核";
                    break;
                case "1":
                    sName = "审核通过";
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            ProjectTpinfoModel m = service.GetList().FirstOrDefault(p=>p.PROJID==id);
            if (m.STATUS < 0 || m.STATUS == 2)
            {
                btnStrs += string.Format("<a  href='ProjectRegister3.aspx?Cmd=Edit&PROJID={0}'><img alt=\"\" src=\"/Resources/Common/img/icon/application_edit.png\" border=\"0\" /></a>", id);
                btnStrs += "&nbsp;&nbsp;|&nbsp;&nbsp;";
                btnStrs += string.Format("<a  onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectRegister3.aspx?Cmd=Delete&PROJID={0}'><img alt=\"\" src=\"/Resources/Styles/icon/delete.gif\" border=\"0\" /></a>", id);
            }
            else {
