...
{
    public partial class ProjectInfoView : Enterprise.Service.Basis.BasePage
    {
        ...
        private void OnCommand()
        {
            //添加操作按钮
            if(lb=="1")
            CreateBT.AddButton("back.gif", Trans("返回"), "ProjectInfoList2.aspx", Utility.PopedomType.List, HeadMenu1);
            if (lb == "2")
        ...
        private void OnStart()
        {
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ProjectInfoView.aspx?Cmd=Edit&PROJID=" + Hid_CKID.Value, false);
                }
            }
            else if (Cmd == "Audit") {
                OnBindData();
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectInfoView.aspx?Cmd=Delete2&FID={0}&PROJID={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
        ...
        private string sendMessageToRecv(ProjectInfoModel model)
        {
            ...
            bfCtrl.SendNotificationMessage(
                ...
                string.Format("/Modules/App/Project/ProjectInfoView.aspx?Cmd=Audit&PROJID={0}", model.PROJID),//消息内容
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            Utility.ShowMsgThenReloadTree(this.Page, "系统提示", ((isOk) ? "操作成功!" : "操作失败!!!"), "ProjectInfoView.aspx?PROJID=" + ckId + "&Lb=" + lb + "&Cmd=Edit");
