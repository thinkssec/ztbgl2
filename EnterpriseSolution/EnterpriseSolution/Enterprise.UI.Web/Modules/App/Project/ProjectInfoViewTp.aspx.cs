...
{
    public partial class ProjectInfoViewTp : Enterprise.Service.Basis.BasePage
    {
        ...
        private void OnCommand()
        {
            //添加操作按钮
            if(lb=="1")
            CreateBT.AddButton("back.gif", Trans("返回"), "ProjectInfoList3.aspx", Utility.PopedomType.List, HeadMenu1);
            if (lb == "2")
                CreateBT.AddButton("back.gif", Trans("返回"), "ProjectZbwjBz.aspx", Utility.PopedomType.List, HeadMenu1);
            if (lb == "3")
                CreateBT.AddButton("back.gif", Trans("返回"), "ProjectRunning.aspx", Utility.PopedomType.List, HeadMenu1);
            if (lb == "4")
                CreateBT.AddButton("back.gif", Trans("返回"), "ProjectTeam.aspx", Utility.PopedomType.List, HeadMenu1);
        ...
        private void OnStart()
        {
            ...
            if (Cmd == "New") {
                ...
                if (cm == null)
                {
                    ...
                    //Txt_SQLY.Text = cm.SQLY.ToRequestString();
                    Lb_SQLY.Text = cm.SQLY.ToRequestString();
                    Txt_BZ.Text = cm.BZ;
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ProjectInfoViewTp.aspx?Cmd=Edit&PROJID=" + Hid_CKID.Value, false);
                }
            }
            else if (Cmd == "Audit") {
                OnBindData();
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectInfoViewTp.aspx?Cmd=Delete2&FID={0}&PROJID={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
        ...
        private void OnBindData()
        {
            ...
            if (q != null)
            {
                ...
                //Txt_SQLY.Text = q.SQLY.ToRequestString();
                Lb_SQLY.Text = q.SQLY.ToRequestString();
                Txt_BZ.Text = q.BZ;
        ...
        private string sendMessageToRecv(ProjectTpinfoModel model)
        {
            ...
            bfCtrl.SendNotificationMessage(
                ...
                string.Format("/Modules/App/Project/ProjectInfoViewTp.aspx?Cmd=Audit&PROJID={0}", model.PROJID),//消息内容
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            Utility.ShowMsgThenReloadTree(this.Page, "系统提示", ((isOk) ? "操作成功!" : "操作失败!!!"), "ProjectInfoViewTp.aspx?PROJID=" + ckId + "&Lb=" + lb + "&Cmd=Edit");
