...
{
    public partial class ProjectRegister : Enterprise.Service.Basis.BasePage
    {
        ...
        {
            //添加操作按钮
            if(Cmd=="Edit")CreateBT.AddButton("back.gif", Trans("返回"), "ProjectInfoList.aspx", Utility.PopedomType.List, HeadMenu1);
        }
        ...
        private void OnStart()
        {
            ...
            if (Cmd == "New") {
                ...
                if (cm == null)
                {
                    ...
                    //cm.NKBDD = "湖北省武汉市东湖新技术开发区光谷大道126号天然气武汉调控中心 室";
                    cm.NKBDD = "";
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ProjectRegister.aspx?Cmd=Edit&PROJID=" + Hid_CKID.Value, false);
                }
            }
        }
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectRegister.aspx?Cmd=Delete2&FID={0}&PROJID={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
        ...
        private string sendMessageToRecv(ProjectInfoModel model)
        {
            ...
            bfCtrl.SendNotificationMessage(
                ...
                string.Format("/Modules/App/Project/ProjectAudit.aspx?Cmd=Audit&PROJID={0}", model.PROJID),//消息内容
