...
{
    public partial class ProjectRegister6 : Enterprise.Service.Basis.BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                User_SPR51.DataSource = list;
                User_SPR51.DataTextField = "UNAME";
                User_SPR51.DataValueField = "USERID";
                User_SPR51.DataBind();
                OnStart();
                OnCommand();
            }
        ...
        private void OnStart()
        {
            ...
            if (Cmd == "New") {
                ...
                if (cm == null)
                {
                    ...
                    cm.NKBDD = "湖北省武汉市东湖新技术开发区光谷大道126号";
                    ...
                    User_Submitter.UserSelectValue = cm.SUBMITTER.ToRequestString();
                    Ddl_ZJLY.SelectedValue = cm.ZJLY;
                    Ddl_ZJBZ.SelectedValue = cm.ZJBZ;
                    Ddl_YWBM.SelectedValue = cm.YWBM.ToRequestString();
                    User_SPR51.SelectedValue = cm.SPR5.ToRequestString();
                    Lb_SUBMITTER.Text = SysUserService.GetUserName(cm.SUBMITTER.Value);
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ProjectRegister6.aspx?Cmd=Edit&PROJID=" + Hid_CKID.Value, false);
                }
            }
        }
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ProjectRegister6.aspx?Cmd=Delete2&FID={0}&PROJID={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
...

        {
            ...
            if (q != null)
            {
                ...
                User_Submitter.UserSelectValue = q.SUBMITTER.ToRequestString();
                Ddl_YWBM.SelectedValue = q.YWBM.ToRequestString();
                User_SPR51.SelectedValue = q.SPR5.ToRequestString();
        ...
        protected void BtnSave_OnClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                entity.SUBMITTER = User_Submitter.UserSelectValue.ToInt();
                ...
                entity.DEPTID = User_Submitter.DeptSelectValue.ToInt();
                entity.KINDNAME = Ddl_KIND.Text;
                entity.YWBM = Ddl_YWBM.SelectedValue.ToInt();
                entity.SPR5 = User_SPR51.SelectedValue.ToInt();
                entity.RTN = 666666;
                SysUserService suer = new SysUserService();
                SysUserModel sss = suer.GetList().Where(w=>w.DEPTID==User_Submitter.DeptSelectValue.ToInt()&&w.ULOGINPASS=="1").FirstOrDefault();
                entity.SHR = sss.USERID;
                sss=suer.GetList().Where(w=>w.DEPTID==Ddl_YWBM.SelectedValue.ToInt()&&w.ULOGINPASS=="1").FirstOrDefault();
                entity.SPR2 = sss.USERID;

                int ddddd = 140;
                //string eeeee = "财务资产部";
                if (Ddl_ZJLY.SelectedValue == "投资")
                {
                    ddddd = 139;
                    //eeeee = "计划经营部";
                }
                sss = suer.GetList().Where(w => w.DEPTID == ddddd && w.ULOGINPASS == "1").FirstOrDefault();
                entity.SPR3 = sss.USERID;

                SysUserDutyService Service = new SysUserDutyService();
                SysDutyService sd = new SysDutyService();
                SysDutyModel gfg = sd.GetList().Where(p => p.DNAME == "招投标管理办公室").FirstOrDefault();
                SysUserDutyModel tt = Service.GetList().Where(p => p.DUTYID == gfg.DUTYID).FirstOrDefault();
                entity.SPR4=tt.USERID;
