...
namespace Enterprise.UI.Web.Modules.App.Project
{
    ...
    /// 设备管理运行主页面
    /// </summary>
    public partial class TeamMain : BasePage
    {
        /// <summary>
        /// EasyUi tree Json
        /// </summary>
        ...
        {
            Enterprise.Service.App.Project.ProjectInfoService psrv=new ProjectInfoService();
            project = psrv.GetList().Where(w => w.PROJID == ProjectId).FirstOrDefault();
            if(!IsPostBack)
            {
                InitTree();
            }
