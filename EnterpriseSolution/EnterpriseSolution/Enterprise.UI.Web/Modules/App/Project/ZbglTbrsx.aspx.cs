...
namespace Enterprise.UI.Web.Modules.App.Project
{
    ...
    /// 项目成果交付页面
    /// </summary>
    public partial class ZbglTbrsx : BasePage
    {
        ...
        public ProjectTbrcbsxService xSrv = new ProjectTbrcbsxService();
        ...
        private void BindCommand()
        {
            ...
            //{
            //    CreateBT.AddButton("new.gif", "新增", "ZbglTbrsx.aspx?Cmd=New&ProjectId=" + projectId + "&NodeCode=" + nodeCode, Utility.PopedomType.List, HeadMenu1);
        ...
        {
            IList<ProjectFwsModel> list = Service.GetList().Where(p=>p.PROJID==projectId&&p.STATUS!=-2).OrderBy(o=>o.CRMINFO).ToList();
            GridView1.DataSource = list;
            GridView1.DataBind();
            ProjectTbrcbsxModel tM = xSrv.GetList().Where(w => w.PROJID == projectId).FirstOrDefault();
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a href='ZbglTbrsx.aspx?Cmd=Edit&ProjectId={0}&Id={1}&NodeCode={2}'><img alt=\"\" src=\"/Resources/Styles/icon/application_edit.png\" border=\"0\"/></a>", projectId, id, nodeCode);
            btnStrs += "&nbsp;&nbsp;|&nbsp;&nbsp;";
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ZbglTbrsx.aspx?Cmd=Delete&ProjectId={0}&Id={1}&NodeCode={2}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", projectId, id, nodeCode);
            return btnStrs;
        }
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            ProjectTbrcbsxModel Model = xSrv.GetList().Where(p => p.PROJID == projectId).FirstOrDefault();
            if (Model == null)
            {
                Model = new ProjectTbrcbsxModel();
            ...
            if (isOk)
            {
                ZbwjService.CreateTbrcbsxb(projectId);

            }
            Utility.ShowMsgThenReloadTree(this.Page, "系统提示", ((isOk) ? "操作成功!" : "操作失败!!!"), "ZbglTbrsx.aspx?ProjectId=" + projectId + "&NodeCode=" + nodeCode);
