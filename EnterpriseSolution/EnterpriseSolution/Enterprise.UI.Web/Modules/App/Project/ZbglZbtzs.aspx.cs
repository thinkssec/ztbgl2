...
namespace Enterprise.UI.Web.Modules.App.Project
{
    ...
    /// 项目成果交付页面
    /// </summary>
    public partial class ZbglZbtzs : BasePage
    {
        ...
        private void BindCommand()
        {
            ...
            //{
            //    CreateBT.AddButton("new.gif", "新增", "ZbglZbtzs.aspx?Cmd=New&ProjectId=" + projectId + "&NodeCode=" + nodeCode, Utility.PopedomType.List, HeadMenu1);
        ...
        private void BindGrid()
        {
            ...
            if (tM != null)
            {
                ...
                if (inM != null)
                    Txt_P003.Text = inM.PROJCONTENT;
            }

            if (inM != null)
            {
                Lb_P002.Text = inM.PROJNAME;

            }
            if (pfL != null && pfL.Count > 0)
                Lb_P001.Text = CrmInfoService.GetCrmName(pfL.First().CRMINFO);
        }
        ...
        {
            ProjectZbtzsModel Model = Service.GetList().Where(p => p.PROJID == projectId).FirstOrDefault();
            //ProjectInfoModel inM = infSrv.GetList().Where(w => w.PROJID == projectId).FirstOrDefault();
            Model.P1 = Txt_P003.Text;
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            btnStrs += string.Format("<a href='ZbglZbtzs.aspx?Cmd=Edit&ProjectId={0}&Id={1}'><img alt=\"\" src=\"/Resources/Styles/icon/application_edit.png\" border=\"0\"/></a>", projectId, id);
            btnStrs += "&nbsp;&nbsp;|&nbsp;&nbsp;";
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ZbglZbtzs.aspx?Cmd=Delete&ProjectId={0}&Id={1}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", projectId, id);
            return btnStrs;
        }
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            ZbwjService.CreateZbtzs(projectId);
            Utility.ShowMsgThenReloadTree(this.Page, "系统提示", ((isOk) ? "操作成功!" : "操作失败!!!"), "ZbglZbtzs.aspx?ProjectId=" + projectId );
