...
{
    public partial class ZyglWgysEdit : Enterprise.Service.Basis.BasePage
    {
        ...
        {
            //添加操作按钮
            CreateBT.AddButton("back.gif", Trans("返回"), "ZyglWgysList.aspx", Utility.PopedomType.List, HeadMenu1);
        }
        ...
        private void OnStart()
        {
            ...
            else if (Cmd == "Delete2")
            {
                ...
                if (q != null)
                {
                    ...
                    Response.Redirect("ZyglWgysEdit.aspx?Cmd=Edit&WID=" + Hid_CKID.Value + "&ZID=" + Hid_ZID.Value, false);
                }
            }
        }
        protected string GetCommandBtn(string id)
        {
            ...
            if(zy.STATUS==4)
            btnStrs += string.Format("<a onclick='return confirm(\"您确定要删除该记录吗?\");' href='ZyglWgysEdit.aspx?Cmd=Delete2&QID={0}&WID={1}&ZID={2}'><img alt=\"\" src=\"/Resources/Common/img/icon/delete.gif\" border=\"0\" /></a>", id,Hid_CKID.Value,Hid_ZID.Value);
            return btnStrs;
        }
        /// <summary>
        /// 当编辑时绑定数据项
        ...
        private string sendMessageToAudit(ZyglWgysModel model)
        {
            string msg = "消息已发送";
        ...
        protected void BtnSave_OnClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ZyglWgysModel entity = service.GetList().FirstOrDefault(p => p.WID == Hid_CKID.Value);
                var zy = pservice.GetList().FirstOrDefault(p => p.ZID == Hid_ZID.Value);
                if (entity == null)
                {
                ...
                {
                    if (entity.STATUS > -1) {
                        Utility.ShowMsg(this.Page, "OK", this.Trans("已提交！无法保存！"), "ZyglWgysList.aspx");
                        return;
                    }
        ...
        protected void BtnAudit_OnClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    if (entity.STATUS > -1)
                    {
                        Utility.ShowMsg(this.Page, "OK", this.Trans("已提交！无法重复！"), "ZyglWgysList.aspx");
                        return;
                    }
            ...
            Response.Redirect("ZyglWgysEdit.aspx?Cmd=Recv&WID=" + Hid_CKID.Value + "&ZID=" + Hid_ZID.Value, false);
