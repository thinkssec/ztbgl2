...
{
    public partial class ZyglWgysList : Enterprise.Service.Basis.BasePage
    {
        ...
        private void BindGrid()
        {
            ...
            list = Service.GetList().Where(p=>p.PSTARTDATE>=st&&p.PENDDATE<=et&&(p.STATUS==5 ||p.STATUS==4)).OrderByDescending(p => p.PSTARTDATE).ToList();
        ...
        public void Bt_Search_OnClick(object sender, EventArgs e)
        {
            ...
            ZyglPlanService Service = new ZyglPlanService();
            var list = Service.GetList().Where(p => (p.STATUS == 5 || p.STATUS == 4)).OrderByDescending(p => p.PSTARTDATE).ToList();
        ...
        protected string GetCommandBtn(string id)
        {
            ...
            if (m.STATUS == 4)
            {
                btnStrs += string.Format("<a  href='ZyglWgysEdit.aspx?Cmd=Edit&ZID={0}'><img alt=\"\" src=\"/Resources/Common/img/icon/application_edit.png\" border=\"0\" /></a>", id);
