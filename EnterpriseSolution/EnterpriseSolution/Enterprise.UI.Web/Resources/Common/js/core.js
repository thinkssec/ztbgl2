...
Core.init = function (update) {
    ...
    var loadShortcut = function () {
        //获取json数组并循环输出每个图标
        $.getJSON("Common/js/shortcut.js", function (sc) {
            $(ul).html("");
            //把返回的json数组存为全局变量
            jsonsc = sc;
...
var GetTaskRight = function (obj) {
    if (!_cache.TaskRight) {
        _cache.TaskRight = $('<div class="popup-menu task-menu" style="z-index:99999;bottom:30px;display:none">' +
                        ...
                        '<tr>' +
								'<td class="ui_main">' +
									'<div class="ui_content">' +
                                       '<ul><li><a menu="close" href="#ta-close"><span class="ui_img">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>关闭</a></li></ul>' +                                       
                                    '</div>' +
								'</td>' +
							'</tr>' +
                           '</tbody>' +
