...
for (var i = 1; i < cols; i++) {
    ...
    for (var j = 1; j < rows; j++) {
        ...
        txt = "<a href='/Modules/App/Jhb/JhbM01_JDBView.aspx?y1=" + year1 + "-01-01&y2=" + year2 + "-12-31&w1=" + w1 + "&w2=" + w2 + "' target='_parent'>" + txt + "</a>";
        $('.htmlgrid tbody tr:eq(' + j + ') td:eq(' + i + ')').html(txt);
    }
}

//修改表头文字 如需俄文翻译 请将此js翻译成俄文即可
for (var i = 1; i < $('.htmlgrid tbody tr th').length; i++) {
    header = $('.htmlgrid tbody tr th:eq(' + i + ')').text();
    header = header.replace("C", "");
    header = "完成率" + header.replace("TO", "~") + "%";
