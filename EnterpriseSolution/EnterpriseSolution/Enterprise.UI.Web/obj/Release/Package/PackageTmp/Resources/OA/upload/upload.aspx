...
protected void Page_Load(object sender, EventArgs e)
{
    ...
    //string attachdir = System.Configuration.ConfigurationManager.AppSettings["xheditorUploadPath"];     // 上传文件保存路径，结尾不要带/
    int dirtype = 1;                 // 1:按天存入目录 2:按月存入目录 3:按扩展名存目录  建议使用按天存
    // 最大上传大小，默认是50M
    int maxattachsize = Enterprise.Component.Infrastructure.Extension.ToInt(System.Configuration.ConfigurationManager.AppSettings["xheditorUploadMaxSize"]);     
    
    string upext = "txt,rar,zip,jpg,jpeg,gif,png,swf,wmv,avi,wma,mp3,mid,xls,pdf,xlsx,ppt,pptx,wps,dot,doc,docx";    // 上传扩展名
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //mod by qw 2013.11.16 统一上传目录 start

                attach_dir = "/Public/Platform/" + DateTime.Now.Year + "/file/" + DateTime.Now.ToString("yyyyMMdd") + "/";
                //attach_dir = attachdir + "/" + attach_subdir + "/";

                // 生成随机文件名
                Random random = new Random(DateTime.Now.Millisecond);
                //filename = DateTime.Now.ToString("yyyyMMddhhmmss") + random.Next(10000) + "." + extension;
                filename = DateTime.Now.ToString("yyyyMMddHHmm") + Guid.NewGuid().ToString().Substring(0, 4) + "." + extension;
